...
-- Select vl_oferta from oferta where vl_oferta >=100000 and vl_oferta <=150000;
...
-- Select nm_vendedor from vendedor where nm_vendedor like '%A';
...
-- Select * from imovel where (cd_vendedor = 4) or vl_preco >=110000 and vl_preco <=200000 order by qt_areautil asc;
...
-- Select imovel.cd_imovel, vendedor.cd_vendedor, vendedor.nm_vendedor, vendedor.sg_estado from imovel INNER JOIN vendedor ON imovel.cd_vendedor = vendedor.cd_vendedor;
-- Select comprador.cd_comprador, comprador.nm_comprador, oferta.cd_imovel, oferta.vl_oferta from comprador INNER JOIN oferta ON comprador.cd_comprador = oferta.cd_comprador;
-- Select imovel.cd_imovel, imovel.vl_preco, imovel.cd_bairro from imovel INNER JOIN vendedor ON imovel.cd_vendedor = vendedor.cd_vendedor where vendedor.cd_vendedor = 3;
-- Select distinct cd_imovel FROM oferta;
-- Select imovel.cd_imovel, oferta.vl_oferta from imovel LEFT JOIN oferta ON imovel.cd_imovel = oferta.cd_imovel;
-- Select comprador.cd_comprador, comprador.nm_comprador, oferta.cd_imovel, oferta.vl_oferta FROM comprador LEFT JOIN oferta ON comprador.cd_comprador = oferta.cd_comprador;
-- Select comprador.cd_comprador, comprador.nm_comprador, oferta.cd_imovel, oferta.vl_oferta from comprador LEFT JOIN oferta ON comprador.cd_comprador = oferta.cd_comprador;
/* Select imovel.ds_endereco, imovel.sg_estado, imovel.cd_bairro, 
  case 
    when imovel.vl_preco < 100000 then 'Preço Baixo'
    when imovel.vl_preco >= 100000 and imovel.vl_preco <= 500000 then 'Preço Médio'
    else 'Preço Alto'
  end as price_level 
from imovel;*/
-- Select vendedor.nm_vendedor, COUNT(*) as total_de_propriedades from imovel INNER JOIN vendedor ON imovel.cd_vendedor = vendedor.cd_vendedor group by vendedor.nm_vendedor order by total_de_propriedades desc;
-- Select MAX(vl_preco) - MIN(vl_preco) as diferenca_de_preco from imovel;
-- Select imovel.cd_vendedor, MIN(vl_preco) as Menor_preco from imovel where vl_preco >= 10000 group by imovel.cd_vendedor;
-- Select oferta.cd_comprador, comprador.nm_comprador, AVG(oferta.vl_oferta) as Média_do_valor_das_ofertas, COUNT(oferta.cd_imovel) as Número_de_ofertas from comprador LEFT JOIN oferta ON comprador.cd_comprador = oferta.cd_comprador group by oferta.cd_comprador, comprador.nm_comprador;